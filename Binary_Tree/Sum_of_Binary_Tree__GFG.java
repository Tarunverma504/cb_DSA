...
*/
import java.util.*;
public class Sum_of_Binary_Tree__GFG {
    ...
    {
        static int sumBT(Node temp){
            //Code
            if(temp==null)
                return 0;
            int leftPart=sumBT(temp.left); // if we get sum of left subtree
            int rightPart=sumBT(temp.right); //ans we get sum of right binary tree
            return leftPart+rightPart+temp.data; //if we add left an right subtree sum and that node it will give tree sum
        }
    }
}
